/* ══════════════════════════════════════════════════════════════════
   PokéVault app.js  v26
   Requires: Supabase JS v2, Chart.js v4, loaded in index.html

   v26 changes:
   • Graded chart system ported from v17: single fetch with
     includeEbay=true, dynamic grade chips from salesByGrade,
     multi-line compare-all chart, full PSA/CGC/BGS/SGC support
   • extractPriceForGrade: precise salesByGrade path replaces
     unreliable deep-scan (fixes graded cards showing raw price)
   • savePortfolioItem: extra graded re-fetch safety net
   • closeOverlay exported to window (fixes share modal close)
   • Share Portfolio removed from metrics grid (fixes 2nd row wrap)
══════════════════════════════════════════════════════════════════ */
...
let _pfChartRangeDays = 90; // Default: 3 months
...
async function loadPortfolioChart() {
  ...
  try {
    ...
    for (let d = new Date(windowStart); d <= windowEnd; d.setDate(d.getDate() + 1)) {
      _pfChartAllDates.push(d.toISOString().split('T')[0]);
    }
...
function _renderPfChart(loading, canvas) {
    ...
    let dates = _pfChartAllDates;
    if (_pfChartRangeDays < 9999) {
      const cutoff = new Date();
      cutoff.setDate(cutoff.getDate() - _pfChartRangeDays);
      const cutoffStr = cutoff.toISOString().split('T')[0];
      dates = _pfChartAllDates.filter(d => d >= cutoffStr);
    }
    if (!dates.length) dates = _pfChartAllDates; // fallback
    ...
    const values = allDates.map(d => _pfChartByDate[d] ?? null);
    ...
    const grad = ctx2d.createLinearGradient(0, 0, 0, _canvas.offsetHeight || 300);
    grad.addColorStop(0,   'rgba(0,229,204,0.28)');
    grad.addColorStop(1,   'rgba(0,229,204,0.02)');

    _portfolioChart = new Chart(_canvas, {
      type: 'line',
      data: {
        labels,
        datasets: [{
...
/* ── Portfolio chart range button handler ── */
function setPfChartRange(days) {
  _pfChartRangeDays = days;
...
window.setPfChartRange = setPfChartRange;
...
//
// v26: ported v17's richer graded chart system:
//   - Single fetch with includeEbay=true gets ALL grade data at once
//   - Dynamic chip buttons rendered from salesByGrade keys (not hardcoded 4 buttons)
//   - Multi-line comparison chart (toggle individual grades or "Compare all")
//   - gradeHistory() builds data points from salesByGrade history or falls back to spot price
//   - Works for both graded (PSA/CGC/BGS) and raw/sealed cards
...
// v27: Only show the grades users care about. BGS BL 10 is conditional (not all cards have it).
// Order matches the dropdown: PSA 10, PSA 9, TAG 10, TAG 9, BGS BL 10, BGS 10, BGS 9
const GRADE_META = {
  psa10:   { label: 'PSA 10',    color: '#f5a623' },
  psa9:    { label: 'PSA 9',     color: '#e8c33a' },
  tag10:   { label: 'TAG 10',    color: '#00e5cc' },
  tag9:    { label: 'TAG 9',     color: '#4dd9c4' },
  bgsBL10: { label: 'BGS BL 10', color: '#ff6bba' },
  bgs10:   { label: 'BGS 10',    color: '#7b61ff' },
  bgs9:    { label: 'BGS 9',     color: '#8070cc' },
  // Keep raw for sealed / ungraded cards
  raw:     { label: 'Raw',       color: '#8c95ad' },
};

// Ordered list for the dropdown (BGS BL 10 is filtered out if unavailable)
const GRADE_DROPDOWN_ORDER = ['psa10','psa9','tag10','tag9','bgsBL10','bgs10','bgs9'];

function gradeKey(conditionOrGrade) {
  if (!conditionOrGrade) return 'raw';
  const s = conditionOrGrade.trim();
  if (/TAG\s*10/i.test(s)) return 'tag10';
  if (/TAG\s*9/i.test(s))  return 'tag9';
  if (/BGS\s*(BL|Black\s*Label|10\.0)/i.test(s)) return 'bgsBL10';
  const m = s.match(/^(PSA|BGS|CGC|SGC)\s+(.+)$/i);
  if (!m) return 'raw';
  const company = m[1].toLowerCase();
  const grade   = m[2].trim().replace('.', '_');
  if (company === 'psa' && grade === '10') return 'psa10';
  if (company === 'psa' && grade === '9')  return 'psa9';
  if (company === 'bgs' && grade === '10') return 'bgs10';
  if (company === 'bgs' && grade === '9')  return 'bgs9';
...
function gradeHistory(salesByGrade, key) {
  ...
  if (raw.length) {
    return raw
      .map(h => ({ date: (h.date||'').split('T')[0], priceSGD: Math.round(Number(h.price ?? h.value ?? 0) * (_fxRate || 1.35) * 100) / 100 }))
  ...
  if (price > 0) {
    const today = new Date().toISOString().split('T')[0];
    return [{ date: today, priceSGD: Math.round(price * (_fxRate || 1.35) * 100) / 100 }];
  }
  return [];
}

// Main function called by openCardDetailModal in index.html
async function loadCardDetailChartV26(item) {
  _cdItem      = item;
  _cdApiResult = null;
  _cdActiveGrades = new Set();
  ...
  const gradeRowEl = document.getElementById('cdm-grade-row');

  if (loadingEl) loadingEl.style.display = 'flex';
  if (canvasEl)  canvasEl.style.display  = 'none';
  if (emptyEl)   emptyEl.style.display   = 'none';
  if (gradeRowEl) gradeRowEl.innerHTML   = '<span style="font-size:11px;color:var(--text3);">Loading…</span>';
  ...
  try {
    ...
    const params   = new URLSearchParams({
      action: isSealed ? 'sealed' : 'search',
      language: lang,
      includeEbay: 'true',
    });
    params.set('name', item.name || item.set_name || '');
    if (!isSealed && (item.set_name || item.set)) params.set('set', item.set_name || item.set || '');

    const res  = await fetch('/api/pokeprice?' + params);
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const data    = await res.json();
    const results = data.results || [];
    if (!results.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, gradeRowEl); return; }

    const r = results[0];
    _cdApiResult = r;
    const fxRate = _fxRate || 1.35;

    // Build raw price history
    const rawHistArr = (r.priceHistory || r.history || [])
      .map(h => ({ date: (h.date||'').split('T')[0], priceSGD: Math.round(Number(h.price ?? h.value ?? 0) * fxRate * 100) / 100 }))
      .filter(h => h.date && h.priceSGD > 0)
      .sort((a, b) => a.date.localeCompare(b.date));

    // If no history, seed with a single spot price today
    if (!rawHistArr.length) {
      const spot = extractPrice(r);
      if (spot != null) {
        rawHistArr.push({ date: new Date().toISOString().split('T')[0], priceSGD: Math.round(spot * fxRate * 100) / 100 });
      }
    }
    ...
    // Determine default active grade
    const myGradeKey = gradeKey(item.condition_or_grade || item.conditionOrGrade || '');
    _cdActiveGrades = new Set();
    ...
    // v27: Build dropdown instead of chips. Only show PSA9/10, TAG9/10, BGS BL10 (if available), BGS10, BGS9.
    // Also include 'raw' for sealed/ungraded items.
    if (gradeRowEl) {
      // Build the ordered list of available options
      const dropdownOptions = [];
      if (rawHistArr.length || isSealed) {
        dropdownOptions.push({ key: 'raw', ...GRADE_META['raw'] });
      }
      for (const k of GRADE_DROPDOWN_ORDER) {
        if (availableGradeKeys.includes(k)) {
          dropdownOptions.push({ key: k, ...GRADE_META[k] });
        }
      }
      ...
      if (dropdownOptions.length === 0) {
        ...
        // Pick a sensible default: prefer the item's own grade, else PSA 10, else first available
        let defaultKey = dropdownOptions[0].key;
        const myKey = myGradeKey && myGradeKey !== 'raw' ? myGradeKey : null;
        if (myKey && dropdownOptions.find(o => o.key === myKey)) defaultKey = myKey;
        else if (dropdownOptions.find(o => o.key === 'psa10')) defaultKey = 'psa10';

        _cdActiveGrades = new Set([defaultKey]);

        gradeRowEl.style.display = 'flex';
        gradeRowEl.style.alignItems = 'center';
        gradeRowEl.style.gap = '10px';
        gradeRowEl.innerHTML = `
          <label style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;white-space:nowrap;">Grade</label>
          <div style="position:relative;display:inline-block;">
            <select id="cdm-grade-dropdown"
              style="appearance:none;-webkit-appearance:none;background:var(--bg3);border:1px solid rgba(0,229,204,.3);border-radius:8px;color:var(--accent);font-size:13px;font-weight:600;font-family:inherit;padding:7px 32px 7px 12px;cursor:pointer;outline:none;min-width:130px;"
              onchange="window.onCdmGradeDropdownChange(this.value)">
              ${dropdownOptions.map(o => `<option value="${o.key}"${o.key === defaultKey ? ' selected' : ''}>${esc(o.label)}</option>`).join('')}
            </select>
            <span style="pointer-events:none;position:absolute;right:10px;top:50%;transform:translateY(-50%);color:var(--accent);font-size:12px;">▾</span>
          </div>
        `;
      }
    }

    _drawCdmChart(rawHistArr, salesByGrade);

  } catch (e) {
    console.warn('loadCardDetailChartV27 error:', e);
    _showCdmEmpty(loadingEl, canvasEl, emptyEl, gradeRowEl);
  }
}
window.loadCdmChart = loadCardDetailChartV26;

function onCdmGradeDropdownChange(key) {
  _cdActiveGrades = new Set([key]);
  const salesByGrade = _cdApiResult?.ebay?.salesByGrade || {};
  _drawCdmChart(_cdRawHistory || [], salesByGrade);
}
window.onCdmGradeDropdownChange = onCdmGradeDropdownChange;

// Keep stubs so any old references don't crash
function toggleCdmGradeChip(key) { onCdmGradeDropdownChange(key); }
window.toggleCdmGradeChip = toggleCdmGradeChip;

function compareAllCdmGrades() {
  // In dropdown mode, just select PSA 10 as a sensible default for "compare"
  const dropdown = document.getElementById('cdm-grade-dropdown');
  if (!dropdown) return;
  const salesByGrade = _cdApiResult?.ebay?.salesByGrade || {};
  _drawCdmChart(_cdRawHistory || [], salesByGrade);
}
window.compareAllCdmGrades = compareAllCdmGrades;

function _drawCdmChart(rawHistArr, salesByGrade) {
  _cdRawHistory = rawHistArr;
  ...
  // Apply time range filter
  const rangeDays = typeof _cdmChartRangeDays !== 'undefined' ? _cdmChartRangeDays : 90;
  ...
  function filterByRange(arr) {
    if (!rangeCutoff) return arr;
    return arr.filter(h => new Date(h.date) >= rangeCutoff);
  }

  const dateSet = new Set();
  if (_cdActiveGrades.has('raw')) filterByRange(rawHistArr).forEach(h => dateSet.add(h.date));
  for (const k of _cdActiveGrades) {
    if (k === 'raw') continue;
    filterByRange(gradeHistory(salesByGrade, k)).forEach(h => dateSet.add(h.date));
  }

  const allDates = [...dateSet].sort();
  if (!allDates.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, null); return; }
  ...
  for (const k of [..._cdActiveGrades].sort()) {
    ...
    let histMap;
    if (k === 'raw') {
      histMap = Object.fromEntries(filterByRange(rawHistArr).map(h => [h.date, h.priceSGD]));
    } else {
      const hist = filterByRange(gradeHistory(salesByGrade, k));
  ...
  if (!datasets.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, null); return; }
...
function _showCdmEmpty(loadingEl, canvasEl, emptyEl, gradeRowEl) {
  if (loadingEl) loadingEl.style.display = 'none';
  if (canvasEl)  canvasEl.style.display  = 'none';
  if (emptyEl)   { emptyEl.style.display = 'block'; emptyEl.textContent = 'No price history data available yet.\nData accumulates as you refresh your portfolio values.'; }
  if (gradeRowEl) gradeRowEl.innerHTML = '';
}

function buildChartConfig(labels, values, label, color) {
  return buildMultiChartConfig(labels, [{ label, values, color: color || '#00e5cc' }]);
}
...
function buildMultiChartConfig(labels, datasets) {
  ...
  return {
    ...
    data: {
      ...
      datasets: datasets.map((ds, i) => {
        ...
        return {
          ...
          data:             ds.values,
          borderColor:      color,
          backgroundColor:  isSingle ? color + '2a' : color + '18',
          borderWidth:      2,
          pointRadius:      0,
          pointHoverRadius: 5,
    ...
    options: {
      ...
      interaction: { mode: 'index', intersect: false },
      plugins: {
        legend: { display: !isSingle, labels: { color: '#aab0c4', font: { size: 11 }, boxWidth: 12 } },
        tooltip: {
          backgroundColor: '#13161f',
          borderColor: 'rgba(0,229,204,.2)',
          borderWidth: 1,
          titleColor: '#8c95ad',
          bodyColor: '#e2e6f0',
          padding: 10,
          callbacks: { label: ctx => ` ${ctx.dataset.label}: SGD $${Number(ctx.parsed.y).toFixed(2)}` },
        },
      },
      scales: {
        x: {
          ticks: { color: '#4e566b', maxTicksLimit: 5, font: { size: 10 } },
          grid: { color: 'rgba(255,255,255,.04)', drawBorder: false },
          border: { display: false },
        },
        y: {
          ticks: { color: '#4e566b', callback: v => `$${Number(v) >= 1000 ? (Number(v)/1000).toFixed(1)+'k' : Number(v).toFixed(0)}`, font: { size: 10 } },
          grid: { color: 'rgba(255,255,255,.04)', drawBorder: false },
          border: { display: false },
        },
      },
...
/* ── Card detail chart time range (v27) ── */
let _cdmChartRangeDays = 90; // default 3M

function setCdmChartRange(days) {
  _cdmChartRangeDays = days;
  // Update active button in cdm time range row
  document.querySelectorAll('#cdm-time-range-row .pf-range-btn').forEach(btn => {
    btn.classList.toggle('active', parseInt(btn.dataset.days) === days);
  });
  // Re-draw with filtered data
  if (!_cdApiResult) return;
  const salesByGrade = _cdApiResult?.ebay?.salesByGrade || {};
  _drawCdmChart(_cdRawHistory || [], salesByGrade);
}
window.setCdmChartRange = setCdmChartRange;
